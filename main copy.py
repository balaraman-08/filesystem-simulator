...
def copy(*args):
    ...
    # No of Required arguments is 2 
    if len(args) != 2:
        print("Invalid usage")
        return False
    else:
        # split the argument by / to get list of paths
        sourceFolderPath = args[0].split('/')
        destinationFolderPath = args[1].split('/')

        sourceFolder = currentFolder
        for folder in sourceFolderPath:
            # If the path contains ..
            # Move to parent folder 
            if folder == "..":
                sourceFolder = sourceFolder.parent
            ...
            sourceFolder = sourceFolder.getChild(folder)
            if not sourceFolder:
                print("Source path not present")
                return False
        
        destinationFolder = currentFolder
        for folder in destinationFolderPath:
            # If the path contains ..
            # Move to parent folder 
            if folder == "..":
                destinationFolder = destinationFolder.parent
                continue
            destinationFolder = destinationFolder.getChild(folder)
            if not destinationFolder:
                print("Destination not present")
                return False
        
        if isinstance(sourceFolder, Folder) and isinstance(destinationFolder, Folder):
            sourceChildren = sourceFolder.getChildren()
            for child in sourceChildren:
                if destinationFolder.isChildPresent(child.name):
                    confirmation = input("{} is already present in Destination. Do you want to overwrite? (Y/n) ".format(child.name))
                    if confirmation.lower() == 'n':
                        continue
                    else:
                        destinationFolder.deleteChildren(child.name)
                destinationFolder.addChild(child)
        elif isinstance(sourceFolder, File) and isinstance(destinationFolder, Folder):
            if destinationFolder.isChildPresent(sourceFolder.name):
                confirmation = input("{} is already present in Destination. Do you want to overwrite? (Y/n) ".format(sourceFolder.name))
                if confirmation.lower() == 'n':
                    print("Operation cancelled")
                    return False
                else:
                    destinationFolder.deleteChildren(sourceFolder.name)
            destinationFolder.addChild(sourceFolder)
            print("Copied successfully")
        else:
            print("Destination is not a folder. Cannot copy")
            return False
